...
//início: trocar texto num elemento selecionado, sem usar função:

//variável para guardar o elemento selecionado, usando o método que usa o 'id':
var paragrafo = document.getElementById('troca');
console.log('O texto original era: ' + paragrafo.textContent);

//variável para guardar um valor textual
var novoTexto = 'Este é um novo texto, inserido pelo javascript';

//inserir o novo valor textual no elemento selecionado:
paragrafo.textContent = novoTexto;
//---fim do script


//início: trocar nome no botão
var botao1 = document.querySelector('#nome');

function atualizaNome() {
	'use strict';
	var nome = prompt('Digite um nome:');
	botao1.textContent = 'Olá, ' + nome + ', como vai?';
}
//final função atualiza nome
...
function criaParagrafo() {
	...
	cria.textContent = 'Voce criou um paragrafo: ';
...
function mensagemHumor() {
	...
	switch (opcaoHumor) {
		case '1':
			...
			break;
		case '2':
			mensagemFinal = 'Calma lá fera?';
			break;
		case '3':
			mensagemFinal = 'Sai daqui Palmeirense';
			break;
		default:
